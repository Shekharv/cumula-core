...
class AdminInterface extends BaseComponent {
	...
	public function startup() {
		...
		$this->addEventListenerTo('Router', 'RouterCollectRoutes', 'collectAdminPages');
		
		// bind to the menu collector event
		$this->addEventListenerTo('MenuManager', 'MenuCollectMenus', 'sendMenus');
		
		//Initialize stats for display on homepage.
		$stats = &$this->stats;
		$this->addEventListenerTo('Application', 'AfterBootStartup', function() use(&$stats) {
			$stats['installed_components'] = count(\I('ComponentManager')->getEnabledComponents());
		});

		$this->addEventListenerTo('Autoloader', 'EventAutoload', 'autoload');

		$this->addEventListener('AdminCollectSettingsPages', array(\I('SystemConfig'), 'setupAdminPages'));
